 ...
 * Alan M. Levine
 * July 12, 2018
 * Heritage: starfpgdif1.c and ?
 *
 * Compute radial coordinates of stars in the focal plane.
 *
 */
...
{
  fprintf(stderr,"usage: %s <fpg_pars.txt file> <stars_fpg.txt file> > output_file\n",
          prog);
  exit(1);
}
...
int main(int argc, char *argv[])
{
  ...
  fileb = argv[2];  // stars file
  fprintf(stderr,"files = %s and %s\n",filea,fileb);
  ...
  for(i=0;i<nst;++i) {
    px[0] = colms[i];
    px[1] = rowms[i];
    iccd = pix_to_mm(xy,px);
    r = sqrt(xy[0]*xy[0] + xy[1]*xy[1]);
    printf("%4d %10.4f %10.4f     %9.5f %9.5f %9.5f\n",i,px[0],px[1],xy[0],xy[1],r);
  }
