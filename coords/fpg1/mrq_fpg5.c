 ...
 *           and also mrq_fpg1.c, mrq_fpg2.c, mrq_fpg3.c, mrq_fpg4.c
 */
...
double **x, **y, **xsig, **ysig, **xmod, **ymod;
double **a, **dxda, **dyda, **covar, **alpha, **beta, **talpha, **tbeta;
double **oneda, **atry, **damax;
double chisq, ochisq;
double alamda, alam0, alamdec, alaminc;
int **lista, **indexa;
...
void mrq_undefine()
{
  ...
  free_matrix(xmod,ndata1);
  free_matrix(ymod,ndata1);

  free_matrix(a,ma1);
  free_matrix(atry,ma1);
  free_matrix(damax,ma1);
...
void mrq_finish()
{
  ...
  if(alamda == 0.0) {
    ...
    fprintf(stderr,"mrq_finish(): Error - alamda = %e is not == 0.0; exiting ...\n",
	    alamda);
    exit(-11);
  }
...
void mrqmin_core()
{
  ...
  if (alamda > 0.0) {
    ...
    for(j=0;j<mfit1;j++) {
      dela = oneda[j][0];
      if(dela > damax[indexa[j][0]][0])
	dela = damax[indexa[j][0]][0];
      else if(dela < -damax[indexa[j][0]][0])
	dela = -damax[indexa[j][0]][0];
      atry[indexa[j][0]][0] = a[indexa[j][0]][0] + dela;
    }
    mrqcof_fpg(1);
    fprintf(stderr,"mrqmin_core(): chisq,ochisq,alamda= %f %f %e\n",chisq,ochisq,alamda);
    if (chisq < ochisq) {
      // alamda *= 0.3;
      alamda *= alamdec;
      ochisq = chisq;
