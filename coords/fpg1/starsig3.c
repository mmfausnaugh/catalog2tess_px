 ...
 *
 * Aug. 5, 2017  - Read and write both unaberrated and aberrated RA and Dec.
 */
...
void read_spx_stars(FILE *fpin, FILE *fpd)
{
  ...
  while(fscanf(fpin,"%d %lf %lf %lf %lf %lf",
	       &starno[i],&ra[i],&dec[i],&ra_ab[i],&dec_ab[i],&tmag[i]) == 6) {
    fscanf(fpin,"%d %lf %lf\n",&ccdno[i],&col[i],&row[i]);
    ++i;
  }
  nst = i;
...
void print_sig_star(int i, FILE *fpo, FILE *fpd, double sx, double sy)
{
  fprintf(fpo,"%4d %11.5f %11.5f %11.5f %11.5f  %9.3f  ",
	  starno[i],ra[i],dec[i],ra_ab[i],dec_ab[i],tmag[i]);
