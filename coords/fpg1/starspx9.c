...
void fitpx_to_napx(double fitpx[2], double napx[2])
{
  int iccd;
  ...
  if( (iccd == 0) || (iccd == 1) ) {
    napx[0] = 45.0 + (fpxhi[iccd] - fitpx[0]);
    napx[1] = 1.0 + (fpyhi[iccd] - fitpx[1]);
  }
  else if( (iccd == 2) || (iccd == 3) ) {
    napx[0] = 45.0 + (fitpx[0] - fpxlo[iccd]);
    napx[1] = 1.0 + (fitpx[1] - fpylo[iccd]);
  }
  else {
    napx[0] = -1.0;  // flags that something isn't right!!
    napx[1] = -1.0;  // ditto
  }
}
...
int main(int argc, char *argv[])
{
  ...
  while(read_star(stdin,stderr,0)==1) {
	...
	fprintf(stdout,"%4d %11.5f %11.5f %11.5f %11.5f    %11.5f %2d %11.4f %11.4f   %11.4f %11.4f\n",
      ...
      else if(strcmp(pixbase,"CCD_AMES") == 0) {
	fitpx_to_napx(fitpx,napx);
	fprintf(stdout,"%4d %11.5f %11.5f %11.5f %11.5f    %11.5f %2d %11.4f %11.4f   %11.4f %11.4f\n",
		i,ra,dec,ra_ab,dec_ab,tmag,iccd+1,napx[0],napx[1],
		SIGX,SIGY);
      }
      else {
	usage(argv[0],argc);
