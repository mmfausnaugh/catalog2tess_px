...
double ra_ab[NSTAR], dec_ab[NSTAR];
int nst, icam;
...
void read_stars(FILE *fpin, FILE *fpd)
{
  ...
  i = 0;
  while(fscanf(fpin,"%lf %lf %lf %lf %lf",
	       &ra[i],&dec[i],&ra_ab[i],&dec_ab[i],&tmag[i]) == 5) {
    // fprintf(fpd,"%f %f %f %f %f\n",ra[i],dec[i],ra_ab[i],dec_ab[i],tmag[i]);
    ++i;
  }
  nst = i;
...
int main(int argc, char *argv[])
{
  ...
  for(i=0;i<nst;++i) {
    // if(i > 3) break;
    dsphcr(dtor*ra_ab[i],dtor*dec_ab[i],vstar);
    ...
    if(star_in_fov(lngd,latd) == 1) {
      ...
      fprintf(stdout,"%4d %11.5f %11.5f %11.5f %11.5f    %11.5f %2d %13.6f %13.6f      %13.6f %13.6f\n",
	      i,ra[i],dec[i],ra_ab[i],dec_ab[i],tmag[i],iccd+1,ccdpx[0],ccdpx[1],
	      fitpx[0]+1.0,fitpx[1]+1.0);
    }
  }
}
/******************************************************************************/
