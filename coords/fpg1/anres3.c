 ...
 *
 * Feb. 7, 2018 - Add command line parameter for outlier detection threshold.
 * Feb. 8, 2018 - Write file containing only the stars with model locations
 *                closer than a spcified threshold to the measured locations.
 */
...
int main(int argc, char *argv[])
{
  ...
  double avgc, avgr, sc, sr, crms, rrms, adst, ad0, delad, rtdel, admax, adthr;
  double adhi;
  FILE *fpout, *fpo2;
  ...
  if(argc != 4) {
    fprintf(stderr,"argc = %d is not 4, exiting...\n",argc);
  }
  nq = atoi(argv[1]);
  adthr = atof(argv[2]);
  adhi = atof(argv[3]);

  fpout = fopen("stars_anr_sel.txt","w");
  fpo2 = fopen("stars_anr_out.txt","w");
  ...
  for(i=0;i<nst;++i) {
    ...
    print_fpg_star(i,fpo2);
    print_fpg_input_star(i,fpout);
  }
  ...
  for(iq=0;iq<nq;++iq) {
	...
	if(rtdel > admax)
	  admax = rtdel;
	if (rtdel >= (adthr*ad0)) {
	  fprintf(stdout,"starno,rtdel,rtdel/ad0= %d %f %f\n",
